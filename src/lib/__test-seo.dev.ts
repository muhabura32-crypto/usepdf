...
async function runTests() {
  ...
  const linkRecommendations = linkingGraph.generateLinkRecommendations('/merge-pdf');
  console.log('   ✅ Link recommendations generated:', linkRecommendations.length, 'links\n');
  ...
  const submittedResult = await indexingOptimizer.submitToGoogleIndexing(
    'https://usepdf.xyz/test',
    'URL_ADDED',
    'high'
  );

  console.log('   ✅ Index submission:', submittedResult ? 'queued' : 'failed');
  ...
  // ✅ FIXED HERE
  behaviorOptimizer.startSession(
    'test-user',
    {
      type: 'desktop',
      os: 'Windows',
